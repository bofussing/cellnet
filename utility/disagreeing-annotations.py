...
# %% # Compare disagreeing annotations on an image
...
def mini_batch(who_n):
    # get all who for files of the pattern QS_7382..{who}.json using regex
    whos = [f.split('..')[1].split('.')[0] for f in os.listdir('./data') if f.startswith('QS_7382..') and f.endswith('.json')]
    who = whos[who_n]
